...
public class TAdminDAO extends HibernateDaoSupport {
	...
	//管理员修改
	public void update(TAdmin persistentInstance) {
		log.debug("updating TAdmin instance");
		try {
			getHibernateTemplate().delete(persistentInstance);
	...
	public TAdmin findById(java.lang.Integer userId) {
		log.debug("getting TAdmin instance with userId: " + userId);
		try {
			TAdmin instance = (TAdmin) getHibernateTemplate().get(
					"com.model.TAdmin", userId);
			return instance;
		} catch (RuntimeException re) {
